...
for f in $(find . -name "*.html" -not -path "./reference/*" -not -name "*antithesis-backup*"); do
  ...
  perl -0pi -e "s{(huisstijl/pagina\.css)(\?v=[0-9a-f]+)?}{\$1?v=$css}g; \
                ...
                s{(huisstijl/blog\.css)(\?v=[0-9a-f]+)?}{\$1?v=$blog}g" "$f"
done
echo "pagina.css=$css pagina.js=$js cyclus.css=$cyc clustertriage.css=$ct clustertriage-v2.css=$ct2 storing.css=$storing blog.css=$blog"
